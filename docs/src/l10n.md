...
```
    src/sg/<utility>/locales/<locale>.ftl
```

Examples:

```
    src/sg/list_directory/locales/en-US.ftl    # Embedded in binary
    src/sg/list_directory/locales/fr-FR.ftl    # Loaded from filesystem
```
...
- In `src/sgcore/src/lib.rs` for **single-call utilities**
