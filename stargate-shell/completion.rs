...
impl Completer for StargateCompletion {
    ...
    fn complete(
        ...
        let cmd_start = line[..start].rfind('|')
            .map(|i| i + 1)
            .unwrap_or(0);
        let cmd_line = &line[cmd_start..];
        let words: Vec<&str> = cmd_line.split_whitespace().collect();
        let cmd_name = words.first().unwrap_or(&"");
        
        let looks_like_path = prefix.starts_with('/') || 
                              prefix.starts_with("./") || 
                              prefix.starts_with("../") || 
                              prefix.starts_with('~');
        
        if looks_like_path {
            let matches = get_path_completions(prefix);
            return Ok((start, matches));
        }
        
        if DIRECTORY_COMMANDS.contains(cmd_name) && words.len() > 1 {
