...
fn find_network_device(interface: &str) -> UResult<Device> {
    if interface == "any" {
        let default_device = Device::lookup()
            .map_err(|e| sgcore::error::USimpleError::new(1, format!("failed to locate the default device: {}", e)))?
            .ok_or_else(|| sgcore::error::USimpleError::new(1, "no network devices found".to_string()))?;
        return Ok(default_device);
    }
    
    let all_devices = Device::list()
        .map_err(|e| sgcore::error::USimpleError::new(1, format!("unable to list devices: {}", e)))?;
    
    all_devices
        .into_iter()
        .find(|d| d.name == interface)
        .ok_or_else(|| sgcore::error::USimpleError::new(1, format!("interface '{}' not found", interface)))
}
...
fn open_packet_capture(device: Device) -> UResult<Capture<pcap::Active>> {
    ...
    Capture::from_device(device)
        .map_err(|e| sgcore::error::USimpleError::new(1, format!("unable to capture: {}", e)))?
        .promisc(promiscuous_mode)
        .snaplen(max_packet_size)
        .open()
        .map_err(|e| sgcore::error::USimpleError::new(1, format!("failed to open device: {}", e)))?
        .setnonblock()
        .map_err(|e| sgcore::error::USimpleError::new(1, format!("failed to set non-blocking mode: {}", e)))
}

#[cfg(target_os = "macos")]
fn capture_packets(interface: &str, max_count: Option<usize>, _verbose: bool) -> UResult<Vec<PacketInfo>> {
    ...
    while captured_count < target_count {
        match capture.next_packet() {
            ...
            Err(e) => {
                return Err(sgcore::error::USimpleError::new(1, format!("error capturing packet: {}", e)));
            }
        }
    }
...
fn capture_packets(_interface: &str, _max_count: Option<usize>, _verbose: bool) -> UResult<Vec<PacketInfo>> {
